...
function resolvedWith(parts: Array<{ source: string; text: string }>): ResolvedAgent {
  return {
    inputPath: "x",
    isDirectory: true,
    rawFrontmatter: {},
...
const codesFor = (
  ...
  venues: Venue[],
): string[] => checkCapabilityDrift(resolvedWith(parts), specWith(venues)).map((i) => i.code);
...
describe("checkCapabilityDrift", () => {
  it("passes clean prose with no capability identifiers", () => {
    expect(codesFor(prose("Momentum trend-following on liquid majors. Skip most cycles."), ["futures"])).toEqual([]);
  });

  it("flags an action whose venue the agent has NOT enabled", () => {
    expect(codesFor(prose("When the edge is strong, take it with pm_open."), ["futures"])).toEqual([
      "drift_venue_not_enabled",
    ]);
  });

  it("accepts the same action when the venue IS enabled", () => {
    expect(codesFor(prose("When the edge is strong, take it with pm_open."), ["futures", "pm"])).toEqual([]);
    expect(codesFor(prose("Place the resting bid via spot_order."), ["spot"])).toEqual([]);
  });
  ...
  it("flags an action the runner does not support at all (rot after a removal)", () => {
    ...
    expect(codesFor(prose("Exit the position with pm_close."), ["futures", "pm"])).toEqual([
      "drift_unknown_action",
    ]);
  });

  it("does NOT flag descriptive snake_case that isn't an action verb", () => {
    expect(codesFor(prose("Watch the spot_price and the futures_curve for divergence."), ["futures"])).toEqual([]);
  });

  it("flags a near-miss cap name in a code span (typo/rename)", () => {
    expect(codesFor(prose("Respect the `maxLevrage` cap on every entry."), ["futures"])).toEqual([
      "drift_unknown_cap",
    ]);
  });

  it("accepts a real cap name in a code span", () => {
    expect(codesFor(prose("Respect `maxLeverage` and `perTradeMarginMusd` and `minConfidence`."), ["futures"])).toEqual([]);
  });

  it("does NOT flag an unrelated camelCase identifier far from any cap", () => {
    expect(codesFor(prose("Size to your `minEdge`, never to excitement."), ["futures"])).toEqual([]);
  });

  it("flags a bare venue identifier in a code span when not enabled", () => {
    expect(codesFor(prose("This tactic only fires on the `pm` venue."), ["futures"])).toEqual([
      "drift_venue_not_enabled",
    ]);
  });
  ...
  it("ignores references in runtime memory (journal/notes.md)", () => {
    ...
    const parts = [{ source: "journal/notes.md", text: "Yesterday I used pm_open and it resolved against me." }];
    expect(codesFor(parts, ["futures"])).toEqual([]);
  });

  it("dedupes a repeated reference within one source", () => {
    expect(codesFor(prose("pm_open here, and pm_open there, and pm_open again."), ["futures"])).toEqual([
      "drift_venue_not_enabled",
    ]);
  });

  it("scans thesis/persona/keystone prose, not just skills", () => {
    const parts = [{ source: "character/thesis.md", text: "My whole edge is the pm_open at the extreme." }];
...
describe("ACTION_TYPES stays in lockstep with the decision schema", () => {
  ...
  const minimal: Record<string, Record<string, unknown>> = {
    futures_open: { type: "futures_open", symbol: "BTC", side: "long", leverage: 1, marginMusd: 1 },
    futures_close: { type: "futures_close", positionId: 1 },
    futures_set_sltp: { type: "futures_set_sltp", positionId: 1, stopLossPrice: 1 },
    spot_order: { type: "spot_order", symbol: "BTC", side: "buy", orderType: "market", quantity: 1 },
    spot_cancel: { type: "spot_cancel", orderId: 1 },
    pm_open: { type: "pm_open", source: "k", slug: "s", outcomeExternalMarketId: "o", stakeMusd: 1 },
  };

  it("every ACTION_TYPE is a parseable runner action", () => {
    for (const t of ACTION_TYPES) {
      const r = parseDecision(JSON.stringify({ decision: "act", actions: [minimal[t]] }));
  ...
  it("equals the decision schema's action literals exactly (necessity + sufficiency)", () => {
    ...
    const schemaTypes = actionSchema.options
      .map((o) => (o as { shape: { type: { value: string } } }).shape.type.value)
      .sort();
...
describe("skills ablation helpers", () => {
  ...
  it("dropping skill parts removes only the tactic prose", () => {
    ...
    const ablated = mergeProseParts(parts.filter((p) => !isSkillProseSource(p.source)));
