...
describe("CoinRithmClient", () => {
  it("backs off on 429 (Retry-After) then succeeds", async () => {
    ...
    const c = new CoinRithmClient({ apiKey: "crk_live_x", fetchFn: fetchFn as unknown as typeof fetch, sleepFn });
    const r = await c.me();
  ...
  it("treats 401/403/409/422 as fail-closed (not retried)", async () => {
    for (const status of [401, 403, 409, 422]) {
      ...
      const c = new CoinRithmClient({ apiKey: "k", fetchFn: fetchFn as unknown as typeof fetch, sleepFn: async () => {} });
  ...
  it("URL-encodes the runId on ledger export", async () => {
    ...
    const c = new CoinRithmClient({ apiKey: "k", fetchFn: fetchFn as unknown as typeof fetch });
